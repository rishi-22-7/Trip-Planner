...
import { Plane, Menu, X, LogOut, User, History } from 'lucide-react';
    ...
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-sm border-b border-slate-200 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Brand */}
          <Link to={isAuthenticated ? '/trips' : '/'} className="flex items-center gap-2 group" data-testid="nav-brand">
            <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center transition-transform group-hover:scale-110 group-hover:rotate-3">
              <Plane className="w-4 h-4 text-white" strokeWidth={2} />
            </div>
            <span className="font-bold text-slate-900 text-lg group-hover:text-blue-600 transition-colors">Trip Planner</span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-3">
            {!isAuthenticated ? (
              <>
                ...
                <Link to="/register" className="text-sm font-semibold bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition-all hover:scale-105 hover:shadow-md">Register</Link>
              </>
            ) : (
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 text-sm font-medium text-slate-600 hover:text-red-500 transition-colors"
                  data-testid="nav-logout"
                >
                  <LogOut className="w-4 h-4 transition-transform hover:-translate-x-0.5" />
                  Logout
                </button>
              </div>
            )}
          ...
          <button
            ...
            className="md:hidden p-2 text-slate-600 hover:text-blue-600 transition-colors hover:bg-slate-50 rounded-lg"
        ...
        {mobileMenuOpen && (
          <div
            className="md:hidden border-t border-slate-100 py-3 space-y-1 animate-fade-in-down"
...
const MobileLink = ({ to, label, onClick }) => (
  <Link
    to={to}
    onClick={onClick}
    className="block px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 hover:text-blue-600 rounded-lg transition-all hover:translate-x-1"
  >
    {label}
  </Link>
);
